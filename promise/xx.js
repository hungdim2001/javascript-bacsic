...
getComments(comments)
            ...
            .then(function(users) {
                return getAvatarsById(userIds, avatars)
                    .then(function(avatars) {
                        return {
                            userx: users,               // Chỗ này vẫn bị
                            avatars: avatars,           // CallBack hell
                            commentx: comments          // Hay gọi khác là Pyramid of doom
                        };
                    })
    ...
    .then(function(data) {
        ...
        var html = '';
        data.commentx.forEach(function(comment) {
            var use = data.userx.find(function(user) {
                return user.id === comment.user_id;
            });
            var avatars = data.avatars.find(function(avatar) {
                return avatar.user_id === comment.user_id;
            })
            html += `<li>${avatars.link} - ${use.name} : ${comment.content}</li>`;
        });
